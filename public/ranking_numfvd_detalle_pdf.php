<?php
/**
 * PDF personalizado: detalle de ranking por atleta (NUMFVD).
 */
...
$html .= '<div class="resumen-carnet">Carnet <strong>' . (int) ($atleta['numfvd'] ?? 0) . '</strong></div>';
...
try {
    ...
    $fn = 'ranking_numfvd_' . $numfvd . '_' . $genero . '_' . date('Y-m-d') . '.pdf';
    $report->generate($fn, true);
} catch (Throwable $e) {
    error_log('ranking_numfvd_detalle_pdf: ' . $e->getMessage());
